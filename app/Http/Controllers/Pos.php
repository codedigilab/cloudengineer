...
class Pos extends Controller
{
    ...
    {
        $response = Http::post($this->apiUrl, ['action' => 'read7']);
    ...
    public function storedata()
    {
        $response = Http::post($this->apiUrl, ['action' => 'read7']);
        $data = json_decode($response->body(), true);
        return view('admin.addpos', compact('data'));
    }
    public function store(Request $request)
    {
        ...
        return redirect()->route('pos.index')->with('success', 'Record Added');
    }
    ...
    public function edit($row)
    {
        ...
        if (!isset($data[$row - 1])) {
            return redirect()->route('admin.pos')->with('error', 'Record not found.');
        }

        $record = $data[$row - 1]; // Get specific row
        return view('admin.editpos', compact('record', 'row'));
    }
    ...
    public function update(Request $request, $row)
    {
        ...
        return redirect()->route('pos.index')->with('success', 'Record Updated');
    }

    public function toggleStatus($row)
    {
        $response = Http::post($this->apiUrl, [
            'action' => 'toggle_status7',
            'row' => $row,
        ]);
        return redirect()->route('pos.index')->with('success', 'Status Updated');
    }
}